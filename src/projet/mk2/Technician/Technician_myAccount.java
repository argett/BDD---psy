...
public class Technician_myAccount extends javax.swing.JFrame{    
    public Technician_myAccount() {       
        initComponents();   
        for(Technician tech : User.getTechnicians()){    
            if(tech.getLogin().equals(Technician.user_connected)){       
                //load all data in text fields  
                label_id.setText(Technician.user_connected);
                text_address.setText(tech.getAddress());
                text_mail.setText(tech.getEmail());
                text_password.setText(tech.getPassword());
                text_department.setText(tech.getDepartment());
                text_phone.setText(tech.getPhone());
                
                if(tech.getIDCN() != -1){
                    text_idcn.setText(String.valueOf(tech.getIDCN()));
                } else {                    
                    text_idcn.setText("");
                }
                break;
    ...
    private void btn_acceptModifActionPerformed(java.awt.event.ActionEvent evt) {                                                
        for(Technician tech : User.getTechnicians()){    
            if(tech.getLogin().equals(Manager.user_connected)){           
                //load all data in text fields  
                if(text_idcn.getText().equals("")){
                    tech.setIDCN(-1);  
                } else { //check what has been entered
                    try { tech.setIDCN(Long.valueOf(text_idcn.getText()));
                    } catch (NumberFormatException ex) {
                        tech.setIDCN(-1);  
                    }
                }
                
                tech.setPhone(text_phone.getText());
                tech.setAddress(text_address.getText());
                tech.setEmail(text_mail.getText());
                tech.setPassword(text_password.getText());
                tech.setDepartment(text_department.getText());
                break;
            }
        }
